...
The final experimental protocol was designed to compare all major synthetic-model variants under a single unified benchmark. Since not all model families were available for exactly the same intermediate feature sources, the benchmark was restricted to the intersection of samples shared by all compared models. This procedure yielded a final dataset of 15,064 source images, which was partitioned into 12,804 development samples and 2,260 held-out test samples, corresponding to an 85/15 split. The split was deterministic and followed the harder detector-like benchmark setting used in the later stages of the project, ensuring that all candidate models were evaluated on the same source-image universe.

To preserve balance across operating conditions, the split was stratified using a composite key defined by distance range, weather condition, and time of day. Distances were grouped into three coarse regimes: near ($z \leq 60$ m), mid ($60 < z \leq 100$ m), and far ($z > 100$ m). The held-out test partition was reserved exclusively for final evaluation and was never used during model selection. All design choices, candidate comparisons, and hyperparameter searches were therefore conducted only on the development split using five-fold stratified cross-validation. This separation ensured that the final test result reflected generalization performance rather than repeated tuning on the evaluation set.

Feature preprocessing depended on the regression family. The geometric descriptors were already partially normalized during feature extraction through quantities such as normalized box width, normalized box height, area ratio, aspect ratio, and normalized image position. For the linear baselines, each continuous feature was standardized within the current training fold using the training-fold mean and standard deviation, while categorical metadata such as weather and time of day were encoded using one-hot variables with one reference level omitted. In contrast, the tree-based models operated directly on the tabular feature values without additional scaling. This choice is appropriate because Random Forest and XGBoost are considerably less sensitive to feature scale, whereas the linear models benefit from standardized inputs for stable least-squares estimation.

Hyperparameter tuning was also performed entirely within the development split. For the exact-feature Random Forest baseline, four candidate configurations were compared using out-of-fold \ac{MAE}, spanning shallow to unbounded trees while varying the tree depth and minimum leaf size. For the final jitter-aggregated feature family, four Random Forest candidates and four XGBoost candidates were evaluated in the same manner. The best-performing candidates were the deeper variants of both families. Their out-of-fold predictions were then combined by a weighted ensemble, and the blend weight was searched on a dense grid between 0 and 1 with a step size of 0.01. The best development performance was obtained with a weight of 0.17 for the Random Forest component and 0.83 for the XGBoost component, indicating that the boosted model provided most of the predictive power while the Random Forest still contributed complementary information.

Model selection was based exclusively on the lowest out-of-fold \ac{MAE} on the development split rather than on held-out test performance. This rule was applied consistently across the scale-only baseline, the depth-only linear model, the linear depth-plus-geometry model, the stacked linear model, the exact-feature Random Forest, the jitter-aggregated Random Forest, and the final RF--XGBoost ensemble. For the stacked linear baseline, additional precautions were taken to prevent information leakage: the lower depth-only Random Forest was cross-fitted inside each outer training fold before its predictions were supplied to the upper linear regressor. After selecting the best model, it was refit on the full development set and evaluated once on the held-out test set. Under this protocol, the best-performing model was the jitter-aggregated ensemble, which achieved a test \ac{MAE} of 6.01 m on the unified synthetic benchmark.

The evaluation protocol emphasized both overall accuracy and robustness across conditions. The primary metric was \ac{MAE}, since it is directly interpretable in meters and reflects the practical localization error of the system. Additional reported measures included mean relative error, \ac{RMSE}, $R^2$, median absolute error, and high-percentile absolute errors. Performance was also analyzed by distance range, and grouped summaries were generated for weather and time-of-day conditions. Finally, the fraction of predictions falling within fixed absolute error tolerances of 5 m, 10 m, and 20 m was reported to quantify how often the system remained within practically acceptable error bands. Together, these metrics provide a more complete characterization of model behavior than a single aggregate score.
