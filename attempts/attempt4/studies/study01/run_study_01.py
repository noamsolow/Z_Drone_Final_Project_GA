"""
Study 01 for attempt4: realistic noisy-bbox rows without aggregation.
...
from attempts.attempt3.studies.study03.run_study_03 import (
    ...
    _jitter_bbox,
    _prepare_heldout_lower_cache,
    _prepare_outer_fold_caches,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run attempt4 Study 01 noisy-bbox expanded-row random-forest study."
    )
    parser.add_argument(
        "--config",
        type=Path,
...
def _read_feature_table(
    ...
    if missing_columns:
        raise ValueError(
            "Existing attempt4 Study 01 feature table is missing expected columns: {}".format(missing_columns)
        )
...
def _validate_existing_feature_rows(
    ...
    for row in existing_rows:
        ...
        if row_id in seen_row_ids:
            raise ValueError(
                "Existing attempt4 Study 01 feature table contains duplicate augmented_row_id values. "
    ...
    if incompatibility_messages:
        raise ValueError(
            "Existing attempt4 Study 01 feature table is incompatible with the current config ({}) . "
...
def _build_or_resume_feature_table(
    ...
    if verbose:
        print(
            "[attempt4-study01] Noisy bbox expanded-row setup\n"
            "  config: {}\n"
            "  dataset_root: {}\n"
            "  total_images: {}\n"
    ...
    if missing_samples:
        ...
        if verbose:
            print(
                "[attempt4-study01] Loading depth model\n"
                "  model_name: {}\n"
                "  device: {}\n"
    ...
    for sample_index, sample in enumerate(missing_samples, start=1):
        ...
        if verbose and (
            ...
            print(
                "[attempt4-study01]   saved {} jitter rows | rows now {}".format(
                    rows_written,
                    len(all_rows),
                )
            )
...
def _load_or_create_group_splits(
    ...
    if test_split_path.exists() and cv_folds_path.exists():
        ...
        print(
            "[attempt4-study01] Rebuilding cached splits because {}".format(
                invalidation_reason
            )
        )
...
def run_study(
    ...
    selected_lower_model_name = "study01_jittered_small_rf"
    ...
    if verbose:
        print(
            "[attempt4-study01] Expanded jitter-row big-RF study\n"
            "  config: {}\n"
            "  total_rows: {}\n"
            "  dev_rows: {}\n"
            "  test_rows: {}\n"
    ...
    for model_index, model_spec in enumerate(model_specs, start=1):
        if verbose:
            print(
                "[attempt4-study01] CV evaluation {}/{} | {} | uses_lower_model={}".format(
                    model_index,
                    len(model_specs),
        ...
        for candidate_index, candidate_name in enumerate(model_spec["candidate_names"], start=1):
            ...
            if verbose:
                print(
                    "[attempt4-study01] Evaluating candidate {}/{} | {} | {}".format(
                        candidate_index,
        ...
        if verbose:
            print(
                "[attempt4-study01] Selected best candidate | {} | {} | oof_mae={:.4f}".format(
                    model_spec["model_name"],
    ...
    if verbose:
        print(
            "[attempt4-study01] CV winner selected | {} | {} | oof_mae={:.4f}".format(
                winner_model_name,
    ...
    for model_index, model_spec in enumerate(model_specs, start=1):
        ...
        if verbose:
            print(
                "[attempt4-study01] Held-out test fit {}/{} | {} | candidate={}".format(
                    model_index,
                    len(model_specs),
                    model_spec["model_name"],
                    best_candidate["candidate_name"],
        ...
        if verbose:
            print(
                "[attempt4-study01] Completed held-out test fit | {} | candidate={} | test_mae={:.4f}".format(
                    model_spec["model_name"],
                    best_candidate["candidate_name"],
    ...
    if verbose:
        print(
            "[attempt4-study01] Study complete\n"
            "  winner_model: {}\n"
            "  winner_candidate: {}\n"
            "  winner_test_mae: {:.4f}\n"
            "  summary_json: {}".format(
